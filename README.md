...
> 🌐 **网页版**：[https://lmqyt.github.io/](https://lmqyt.github.io/) —— 无需安装 App，浏览器打开即可阅读。
>
> **说明**：网页版与 App 版本**不同**，只实现**基础阅读功能**（导入、书架、翻页、进度记忆、主题），不带 App 的沉浸式 / 手势翻页 / 原生存储等增强能力；好处是**不限平台**——手机、平板、电脑，只要有浏览器随时都能打开读。**网页版同样支持进度记忆**：阅读位置保存在浏览器本地，下次打开自动回到上次的章节与页面；后续计划打通**云同步**，实现多端进度与书架互通。
...
- **手机模式工具栏**：竖屏时顶部按钮收进 ☰ 下拉菜单（背景色、主题、单双页、沉浸、目录、书架）；横屏/平板仍用完整工具栏
- **阅读设置**：主题（深色/浅色）、背景色（白色/浅绿/类纸/🎨自定义颜色）、单双页布局、目录

## 当前未实现（待后续完善）

- **翻页动画**：未实现翻页过渡动画（当前直接切换，无淡入/滑动效果）
- **字体设置**：字号与字体切换暂不可用（界面有入口，功能待修复）
